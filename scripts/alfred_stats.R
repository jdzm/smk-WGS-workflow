...
for(sid in unique(all$Sample)) {
    for(rg in unique(all[all$Sample == sid,]$Library)) {
           ...
           p1=p1 + theme(legend.position="bottom", legend.direction='horizontal') + theme_bw()
               print(p1)
    }
}
print(warnings())       
...
for(sid in unique(all$Sample)) {
   for(rg in unique(all[all$Sample == sid,]$Library)) {
    ...
    p1=p1 + scale_x_continuous(labels=comma)
    p1=p1 + theme(axis.text.x = element_text(angle=45, hjust=1))+ theme_bw()
    print(p1)
     }
   }
}
...
if (!is.null(all)) {
   ...
   for(sid in unique(all$Sample)) {
    ...
    p1=p1 + facet_wrap(~ Sample) + theme(legend.position="bottom", legend.direction='horizontal')+ theme_bw()
    print(p1)
     }
   }
   print(warnings())
...
if (!is.null(all)) {
   ...
   for(sid in unique(all$Sample)) {
    ...
    p1=p1 + scale_x_continuous(labels=comma) + xlab("Phased block size") + ylab("Count")+ theme_bw()
        print(p1)
     }
   }
   print(warnings())
...
p1=ggplot(data=me, aes(x=x, y=y)) + geom_blank() + theme_bw() + xlim(0, nc) + theme(line=element_blank(), text=element_blank(), title=element_blank())
