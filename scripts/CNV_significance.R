...
# zz <- file(snakemake@log[["logFile"]], open="wt")
# sink(zz, type="message")


## Runs on single samples
#dataTable <-read.table(snakemake@input[["freec_ratio"]], header=TRUE)
dataTable <-read.table(args[1], header=TRUE)
ratio<-data.frame(dataTable)

#dataTable <- read.table(snakemake@input[["cnvs"]], header=FALSE)
...
write.table(cnvs, file=paste0 (args[3], "freec_CNVs.p.value.txt"),sep="\t",quote=F,row.names=F)
