...
dataTable <-read.table(sample_name, header=TRUE);

ratio<-data.frame(dataTable)
ploidy <- type.convert(args[4])
ploidy = 2
ratio = cnvs %>% as.data.frame()
sample_name = "Wt_Ctrl"


png(filename = paste(sample_name,".log2.png",sep = ""), width = 1180, height = 1180,
...
png(filename = paste(sample_name,".png",sep = ""), width = 1180, height = 1180,
    units = "px", pointsize = 20, bg = "white", res = NA)
plot(1:10)
op <- par(mfrow = c(5,5))
