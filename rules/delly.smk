...
rule delly_call:
	...
	output:
		bcf = temp('%s/{s}/delly/raw_calls.bcf' % (derived)),
		csi = temp('%s/{s}/delly/raw_calls.bcf.csi' % (derived)),
...
rule delly_filter:
	...
	shell:
		...
		# Not in place. just keeping code > -a 0.25 # require a minimum variant allele frequency of 25%
